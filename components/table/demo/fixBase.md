...
- 为表格父元素或者直接在表格上添加类名 `dtinsight-table-fixed-base`
- 计算当前表格以外的元素所占高度（如 300px）
- 再为表格设置`style={{ height: 'calc(100vh - 300px)' }}`
- 特别的，`dtinsight-table-fixed-base` 用于自带分页的表格
- 底部分页通过 footer 实现的表格请使用类名 `dtinsight-table-fixed-contain-footer`
...
ReactDOM.render(
  <div>
    <Table
      ...
      className="dtinsight-table-fixed-base"
